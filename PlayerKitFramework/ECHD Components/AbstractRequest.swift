...
internal extension AbstractRequest {
    ...
    func getHost() -> String {
        return EchdConnectionManager.sharedInstance.echdPortalAddress.url
    }
    ...
    func getHeaders() -> HTTPHeaders {
        ...
        if let cookie = EchdConnectionManager.sharedInstance.getCookie() {
    ...
    func checkCookies(for url: URL, success: (_ sessionId: String?,_ grailsRememberMe: String?) -> Void, fail: (Error) -> Void ) {
