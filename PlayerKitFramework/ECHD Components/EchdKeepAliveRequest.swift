...
class EchdKeepAliveRequest: AbstractRequest {
            ...
            .responseJSON(queue: queue) { [weak self] response in
                ...
                case .success:
                    ...
                    if code == 401 {
                     ...
                     fail(NSError(domain: "EchdKeepAliveRequest::request", code: 401, userInfo: [NSLocalizedDescriptionKey: try? response.result.get()]))
    ...
    private func processHeaders(_ allHeaders: [AnyHashable: Any]) {
        if let headers = allHeaders as? [String: Any],
            ...
            if let indexOfSemicolon = stringWithjSessionIdValue.index(of: ";") {
